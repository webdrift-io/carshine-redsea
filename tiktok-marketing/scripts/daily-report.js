...
(async () => {
  ...
  console.log(`  📱 Found ${posts.length} connected posts across target channels (${targetProviders.join(', ')})\n`);
  ...
  for (const app of apps) {
    ...
    report += `## ${app}\n\n`;
    report += `| Date | Platform | Hook | Views | Likes | Comments | Shares |\n`;
    report += `|------|----------|------|------:|------:|---------:|-------:|\n`;
    
    for (const p of appPosts) {
      const viewStr = p.views > 1000 ? `${(p.views / 1000).toFixed(1)}K` : `${p.views}`;
      report += `| ${p.date} | ${p.platform.toUpperCase()} | ${p.hook.substring(0, 45)}... | ${viewStr} | ${p.likes} | ${p.comments} | ${p.shares} |\n`;
    }
  ...
  for (const app of apps) {
    ...
    // Group and diagnose by platform
    const platformsInApp = [...new Set(appPosts.map(p => p.platform))];
    for (const platform of platformsInApp) {
      const platPosts = appPosts.filter(p => p.platform === platform);
      const avgViews = platPosts.length > 0 
        ? platPosts.reduce((s, p) => s + p.views, 0) / platPosts.length : 0;
      ...
      if (viewsGood && (!hasConversionData || conversionGood)) {
        ...
        report += `- Average ${Math.round(avgViews).toLocaleString()} views per post on ${platform.toUpperCase()}\n`;
        report += `- Make 3 variations of the top-performing hooks\n`;
        report += `- Test different posting times for optimization\n`;
        report += `- Ensure cross-posting is active\n`;
      } else if (viewsGood && hasConversionData && !conversionGood) {
        report += `🟡 **Views good + Conversions poor** → FIX THE CTA\n`;
        report += `- People are watching (avg ${Math.round(avgViews).toLocaleString()} views on ${platform.toUpperCase()}) but not converting\n`;
        ...
        report += `- People who see it convert, but not enough see it (avg ${Math.round(avgViews).toLocaleString()} views on ${platform.toUpperCase()})\n`;
  ...
  if (allHooks.length > 0 && hasRC) {
    ...
    for (const h of sorted) {
      ...
      const platStr = h.platform ? `[${h.platform.toUpperCase()}] ` : '';
      report += `**${platStr}"${h.text.substring(0, 55)}..."** — ${h.views.toLocaleString()} views, ${h.conversions} conversions\n`;
